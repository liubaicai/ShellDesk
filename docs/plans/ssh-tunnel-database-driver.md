...
**Tech Stack:**
- SSH: `russh` + `russh-keys`（纯 Rust async，Tokio）
...
**Objective:** 在 `src-tauri/Cargo.toml` 中添加隧道、错误和 URL 编码依赖，锁定兼容 ShellDesk Rust 1.80 的版本范围。
...
# SSH tunnel support. Use 0.53.0 while ShellDesk keeps rust-version = "1.80";
# newer russh releases currently require Rust 1.85+.
russh = "0.53.0"
russh-keys = "0.49.2"
async-trait = "0.1.88"
ssh-key = "0.6.7"
...
- Do not upgrade to `russh = "0.55"` or newer unless `src-tauri/Cargo.toml` `rust-version` is also raised to at least `1.85`.
...
The implementation below is intentionally self-contained and targets the pinned `russh = "0.53.0"` API.
...
- [russh 0.53.0 Cargo metadata](https://docs.rs/crate/russh/0.53.0/source/Cargo.toml.orig) - `rust-version = "1.75"`，兼容 ShellDesk 当前 `rust-version = "1.80"`
- [russh latest crates.io metadata](https://crates.io/crates/russh) - 当前最新发布要求 Rust 1.85+，升级前必须复核 MSRV
