...
- 接入状态：已接入远程桌面目录，appKey 为 `port-manager`；注册、窗口、图标、翻译和渲染分支由合同检查覆盖。
...
端口与监听管理器用于展示远程主机当前开放端口、监听进程和连接状态，也管理按主机保存的 SSH 端口转发。它帮助用户快速判断服务是否监听、端口是否冲突、某个端口由哪个进程占用，并在不依赖系统 OpenSSH 的前提下建立本地、远程或动态 SOCKS5 转发。
...
- 复制端口诊断信息。
- 从端口跳转到进程管理器或服务管理器。
- 保存本地、远程和动态 SOCKS5 转发，并在连接主机后自动启动。
- SSH 转发会话意外断开时查看恢复状态和重试次数。
...
- `src-tauri/src/port_forward.rs` 保存按主机关联的配置、维护运行状态、自动启动和指数退避恢复。
- `src-tauri/src/ssh_tunnel.rs` 通过 `russh` 实现本地 `direct-tcpip`、服务端 `tcpip-forward` 和 SOCKS5 CONNECT；不调用系统 `ssh -L/-R/-D`。
- 配置写入应用数据目录下的私有 `port-forwards.json`，使用原子替换；运行时与当前连接 ID 绑定，连接关闭即取消。
- 本地与远程监听默认只允许回环地址；非回环监听必须在配置中显式确认。
- `src/components/remote-desktop/RemotePortForwarding.tsx` 提供配置、启停、状态、重试和两步删除界面。
...
- PID 缺失时 UI 不崩溃。
- 本地、远程与动态转发可以保存并手动启停，端口 `0` 使用服务端或本地自动分配的端口。
- 自动启动只匹配当前持久化主机 ID；关闭连接会停止对应运行时。
- 非回环监听未显式确认时后端拒绝保存。
- SSH 转发断开后在启用恢复时显示 `恢复中` 和重试次数，禁用恢复时保留错误状态。
