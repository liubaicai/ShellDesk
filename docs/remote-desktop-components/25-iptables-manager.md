...
- 接入状态：已接入远程桌面目录，appKey 为 `iptables-manager`；注册、窗口、图标、翻译和渲染分支由合同检查覆盖。
- 能力声明：平台 `linux`；模式 `management`；权限 `sudo-required`。
- 依赖探测：Linux `iptables`；Windows `iptables`；macOS `iptables`。存在依赖时由 Launchpad 连接级探测并显示缺失原因。
...
首版复用 `window.guiSSH.connections.runCommand`，不新增后端 IPC。
