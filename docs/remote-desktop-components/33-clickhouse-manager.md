...
SQL 生成、CREATE TABLE 解析、列差异与导入目标编码位于 `clickHouseWorkbenchModel.ts`；数据库编辑器、CSV/JSON 解析和导入弹窗分别复用 `databaseEditorExtensions.ts`、`databaseImportUtils.ts`、`DatabaseImportDialog.tsx`。
