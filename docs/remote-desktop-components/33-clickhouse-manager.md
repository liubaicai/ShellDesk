...
- 接入状态：已接入远程桌面目录，appKey 为 `clickhouse`；注册、窗口、图标、翻译和渲染分支由合同检查覆盖。
...
SQL 生成、CREATE TABLE 解析、列差异与导入目标编码位于 `clickHouseWorkbenchModel.ts`；数据库编辑器复用 `databaseEditorExtensions.ts`，CSV/JSON 解析、草稿状态和导入弹窗统一复用 `database-import/`。
