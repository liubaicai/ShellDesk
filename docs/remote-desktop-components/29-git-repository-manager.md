...
- 接入状态：已接入远程桌面目录，appKey 为 `git-manager`；注册、窗口、图标、翻译和渲染分支由合同检查覆盖。
- 能力声明：平台 `linux / windows / macos`；模式 `management`；权限 `user`。
- 依赖探测：Linux `git`；Windows `git`；macOS `git`。存在依赖时由 Launchpad 连接级探测并显示缺失原因。
...
首版复用 `runCommand`，不新增后端 IPC。路径、分支和文件参数在渲染进程构造命令前校验并分别使用 POSIX shell / PowerShell 引号处理。
