...
SQL 生成、schema 建模和导入目标解析位于 `postgresWorkbenchModel.ts`；数据库编辑器、CSV/JSON 解析和导入弹窗分别复用 `databaseEditorExtensions.ts`、`databaseImportUtils.ts`、`DatabaseImportDialog.tsx`。
