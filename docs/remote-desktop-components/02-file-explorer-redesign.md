...
- 接入状态：已接入远程桌面目录，appKey 为 `files`；注册、窗口、图标、翻译和渲染分支由合同检查覆盖。
...
- 文件行操作样式必须以 `.explorer-row` 为边界，避免通用 `.row-actions` 规则污染 SFTP 传输窗口等复用相同类名的表格。
...
- 文件管理器的行操作样式不会改变 SFTP 传输表格的单元格布局和滚动范围。
