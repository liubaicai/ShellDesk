...
- 持久化白名单：`src-tauri/src/vault/remote_profiles.rs` 与 `src-tauri/src/vault/normalize.rs`。
...
| `backup-manager` | 备份管理 | `RemoteBackupManager.tsx` | `44-backup-manager.md` |
...
- 在 `src-tauri/src/vault/remote_profiles.rs` 与 `src-tauri/src/vault/normalize.rs` 更新白名单和目录版本。
