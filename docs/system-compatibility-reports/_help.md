...
``` shell
# mongodb
podman run -d --name mongodb -p 27017:27017 -e MONGO_INITDB_ROOT_USERNAME=root -e MONGO_INITDB_ROOT_PASSWORD=password mongo:latest
```
...
``` shell
# elasticsearch
podman run -d  --name elasticsearch  -p 9200:9200 -p 9300:9300  -e "discovery.type=single-node"   -e "ES_JAVA_OPTS=-Xms1g -Xmx1g" elasticsearch:8.17.4
...
``` shell
# vnc-server
podman run -d --name my-vnc-container -p 5900:5901 -p 6900:6901 -e VNC_PW=password accetto/ubuntu-vnc-xfce-g3
```

``` shell
# minio
podman run -d --name minio --restart always -p 9000:9000 -p 9090:9090 -e MINIO_ROOT_USER=admin -e MINIO_ROOT_PASSWORD=password minio/minio server /data --console-address ":9090"
