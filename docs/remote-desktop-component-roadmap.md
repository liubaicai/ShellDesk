...
截至 2026-07-26，ShellDesk 远程桌面已经接入 43 个远程桌面 appKey，另有 1 个设置页登录会话面板。本文档用于说明当前应用目录、组件文档位置和后续维护规则；单组件细节放在 `remote-desktop-components/`。
...
- 当前 app catalog version 为 `18`。新增 appKey 时必须同步迁移版本和白名单，避免用户拖到桌面的图标被 vault 清洗掉。
