...
截至 2026-07-26，ShellDesk 远程桌面已经接入 44 个远程桌面 appKey，另有 1 个设置页登录会话面板。本文档用于说明当前应用目录、组件文档位置和后续维护规则；单组件细节放在 `remote-desktop-components/`。
...
| --- | --- | --- | --- | --- |
| 01 | `terminal` | 终端 | [终端组件重设计](./remote-desktop-components/01-terminal-redesign.md) | xterm.js 会话、russh PTY 后端、终端工具菜单、首个默认窗口优先使用且启用鼠标滚动的 tmux 会话、命令请求、文件/记事本/AI 协作 |
| 02 | `files` | 文件管理器 | [文件管理器组件重设计](./remote-desktop-components/02-file-explorer-redesign.md) | SFTP 导航、传输、压缩解压、权限、打开方式、路径联动和样式隔离 |
...
- Launchpad 的可用性状态以应用名称旁的低强调色点显示，完整状态和缺失依赖仍由可访问名称与悬浮提示提供。
- 桌面、文件管理器和其他内置应用复用全局加载的右键菜单基础样式；通用菜单定位与层级不得依赖某个按需应用样式先行加载。
