...
- Git 仓库管理器用于查看远程分支状态、变更文件、diff、最近提交，并执行暂存/取消暂存、commit、fetch、pull、push、checkout
- Web 服务管理器覆盖 Nginx、Apache/httpd、Caddy 的配置发现、配置测试、reload 和 restart 流程
...
| Git 仓库 | 远程 Git 状态、变更文件、diff、提交记录、暂存/取消暂存、commit、fetch、pull、push 和 checkout |
| Web 服务 | Nginx、Apache/httpd、Caddy 配置发现、配置测试、reload 和 restart |
