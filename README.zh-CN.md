...
<p align="center">
  ...
  它把 SSH 和本地连接、密钥管理、终端、SFTP、远程编辑器、浏览器、VNC、数据库、WebDAV 同步和系统运维工具收进一个桌面式工作区。
</p>
...
ShellDesk 面向开发者、运维工程师和需要长期维护多台服务器的使用场景。它不是单纯的终端替代品，而是围绕一次 SSH 或本地连接展开的桌面式工作台：连接后，你可以在同一个窗口里打开终端、文件管理、数据库、VNC、内网浏览器、系统监控、日志、服务管理、网络诊断和安全巡检等工具。
...
- 防火墙、iptables、网络诊断、包管理器、计划任务、证书管理、登录会话、安全巡检面向运维排障
