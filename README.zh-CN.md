...
- 支持卡片/表格视图、连接状态筛选、最近活动排序、大规模主机分页和键盘导航，并可在列表旁直接查看连接与系统详情
- 可通过预览式向导迁移 MobaXterm 书签、Xshell 会话、SecureCRT 会话或 CSV 主机清单，并在应用前完成校验、重复项处理、密码明确授权及一次受保护的撤销
- 支持保存密码、私钥或系统 SSH Agent 登录方式、代理/跳板机设置、本地模式，以及连接前凭据补录；Agent 配置不会持久化密码或密钥材料
- 可设置 3–120 秒的全局 SSH 连接超时，并为慢速或不稳定网络中的单台主机单独覆盖
...
- 主窗口提供 SD-Agent 工作区，使用可搜索、虚拟化的主机选择器复用已保存主机上下文处理跨主机任务，并直接跳转到内置工具
...
- WebDAV 同步可在多台设备之间备份和恢复本地 Vault；远端写入必须读回并校验摘要后才推进本地同步基线，严格的格式与版本不变量会拒绝含糊迁移。更新器通过 Tauri 更新流程检查 GitHub Releases
