...
- 系统监视器、进程管理、服务管理、容器管理、Kubernetes 管理、虚拟机管理、端口监听、磁盘分析用于日常巡检
- 虚拟机管理器通过远端 `virsh` 提供 libvirt URI 探测、虚拟机生命周期与快照、虚拟网络、存储池、串口控制台和 VNC 联动
...
ShellDesk 的远程 SSH 路径已经迁移为纯 Rust 实现：

- `russh_client.rs` 负责 SSH 连接建立、主机密钥校验、密码/私钥/agent/键盘交互认证、跳板机/代理传输和命令 exec channel
- `terminal.rs` 负责 russh `request_pty` 远程终端、shell/exec 启动、终端输入和 resize 事件
- `ssh_tunnel.rs` 负责数据库、浏览器代理、VNC 和 HTTP 隧道使用的 `direct-tcpip` 转发，不再有 OpenSSH fallback
- `connection/host_keys.rs` 通过 russh 扫描和分类主机密钥，不再调用 `ssh-keyscan`
- `vault/ssh_keys.rs` 支持导入和生成密钥对，不再调用 `ssh-keygen`

更多后端模块边界、安全约束和维护规则见 [SSH 架构说明](docs/ssh-architecture.md)。
...
│       ├── database/tunnel.rs           # 原生数据库隧道会话、超时和清理 helper
