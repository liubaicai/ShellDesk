...
- A dedicated native dual-pane SFTP workspace opens directly from a host and adds local/remote trees, process-owned queued transfers that survive closing the SFTP window, fingerprint-checked resumable staging with atomic final replacement, pause/retry/cancel, recursive comparison, one-way sync, conflict handling, streamed `russh-sftp` transfers, and a persistent global transfer center shared by every app window
...
- Backup Manager creates and validates file or database backups, downloads or uploads them to S3 / MinIO, restores confirmed archives, and creates Unix or Windows schedules without persisting typed database passwords
...
| `pnpm test` | Runs contract checks, frontend unit tests and build, Playwright UI smoke tests, Rust fmt/clippy/tests, and `cargo check` |
...
| `pnpm check:unit` | Type-checks and runs browser-free TypeScript unit tests |
| `pnpm check:ui` | Type-checks and runs Playwright UI smoke tests |
| `pnpm check:rust` | Runs Rust format checks, clippy with `-D warnings`, and tests |
| `pnpm check:rust:coverage` | Runs the Rust coverage summary and minimum-threshold gate |
