...
- A dedicated native dual-pane SFTP workspace opens directly from a host and adds local/remote trees, process-owned queued transfers that survive closing the SFTP window, pause/retry/cancel, recursive comparison, one-way sync, conflict handling, streamed `russh-sftp` transfers, and a persistent global transfer center shared by every app window
