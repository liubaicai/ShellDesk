...
const exactTranslations = new Map<string, string>(Object.entries({
  ...
  '获取中...': 'Fetching...',
  '获取模型': 'Fetch models',
  '当前只完成全局 AI 配置和模型发现，后续终端、文件、数据库等组件可以复用这套配置接入 AI 能力。': 'This only adds global AI configuration and model discovery for now; later terminal, file, database, and other components can reuse it for AI features.',
  'AI Agent': 'AI Agent',
  'AI Agent 侧边栏': 'AI Agent sidebar',
  '关闭 AI 侧边栏': 'Close AI sidebar',
  '未配置模型': 'No model configured',
  '请先在设置里配置 AI 提供商、API 密钥和默认模型。': 'Configure the AI provider, API key, and default model in Settings first.',
  '探测中...': 'Probing...',
  '重新探测环境': 'Probe again',
  '探测环境': 'Probe environment',
  '带入当前文件内容': 'Include current file content',
  '带入当前文本': 'Use current text',
  '可以让 Agent 生成、重构或解释当前文件。': 'Ask the agent to generate, refactor, or explain the current file.',
...
const patternTranslations: Array<[RegExp, (...matches: string[]) => string]> = [
  ...
  [/^AI 请求失败：(.+)$/u, (message) => `AI request failed: ${translateText(message, 'en-US')}`],
...
const phraseTranslations: Array<[string, string]> = Object.entries({
  ...
  '当前运行环境未提供 AI 对话接口。': 'The current runtime does not provide the AI chat interface.',
  '请先在设置中完成 AI 提供商、API 密钥和模型配置。': 'Finish configuring the AI provider, API key, and model in Settings first.',
  '当前标签是只读状态，无法应用 AI 修改。': 'The current tab is read-only, so AI changes cannot be applied.',
