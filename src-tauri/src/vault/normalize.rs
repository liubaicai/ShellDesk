...
const REMOTE_DESKTOP_APP_CATALOG_VERSION: i64 = 17;
...
pub(crate) fn normalize_app_settings(raw_settings: &Value) -> Result<Value, String> {
    ...
    Ok(json!({
        ...
        "minimizeToTrayOnClose": read_bool(settings.get("minimizeToTrayOnClose"), defaults["minimizeToTrayOnClose"].as_bool().unwrap_or(false)),
        "minimizeToTrayPromptedOnClose": read_bool(settings.get("minimizeToTrayPromptedOnClose"), defaults["minimizeToTrayPromptedOnClose"].as_bool().unwrap_or(false)),
